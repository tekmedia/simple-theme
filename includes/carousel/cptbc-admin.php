...
function cptbc_image_url(){
    ...
    <p class="post-attributes-label-wrapper">
        <label class="post-attributes-label">Ссылка:</label>
        <small><em>(оставьте пустым, если ссылка не требуется)</em></small>
    </p>
	<input name="cptbc_image_url" value="<?php echo $cptbc_image_url; ?>" type="text" />
	
    <p class="post-attributes-label-wrapper">
	<label class="post-attributes-label"><input type="checkbox" name="cptbc_image_url_openblank" <?php if($cptbc_image_url_openblank == 1){ echo ' checked="checked"'; } ?> value="1" />Открывать в новом окне?</label>
    </p>
	
    <p class="post-attributes-label-wrapper">
        <label class="post-attributes-label">Текст ссылки:</label>
        <small><em>(отображается только при наличии ссылки)</em></small>
    </p>
	<input name="cptbc_image_link_text" value="<?php echo $cptbc_image_link_text; ?>" type="text" />
	<?php
}
function cptbc_admin_init_custpost(){
	add_meta_box("cptbc_image_url", "Ссылка слайдера", "cptbc_image_url", "cptbc", "normal", "low");
}
