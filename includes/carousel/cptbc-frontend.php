...
function cptbc_frontend($atts){
	...
	while ( $loop->have_posts() ) {
		...
		if ( '' != get_the_post_thumbnail(get_the_ID(), $atts['image_size']) ) {
			...
			$image = get_the_post_thumbnail( get_the_ID(), $atts['image_size'], $attr );
	...
	if(count($images) > 0){
		ob_start();
		?>
		<div id="cptbc_<?php echo $id; ?>" class="carousel fade" <?php if($atts['use_javascript_animation'] == '0'){ echo ' data-ride="carousel"'; } ?> data-interval="<?php echo $atts['interval']; ?>">
			...
			if( count( $images ) > 1 ){
				...
				<?php } else if($atts['showcontrols'] === 'true' && $atts['twbs'] == '4') { ?>
					<a class="carousel-control-prev" href="#cptbc_<?php echo $id; ?>" role="button" data-slide="prev">
                        <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                        <span class="sr-only">Previous</span>
                    </a>
					<a class="right carousel-control" href="#cptbc_<?php echo $id; ?>" role="button" data-slide="next">
